...
if st.sidebar.button("↺ إعادة ضبط الفلاتر", width='stretch'):
    for k, v in DEFAULTS.items():
        st.session_state[k] = v
    st.rerun()
...
if f.empty:
    st.warning("مفيش بيانات مطابقة للفلاتر المختارة، جرّب توسّع الاختيار شوية.")
    st.stop()
...
c2.metric(
    ...
    delta=f"{f['salary_usd'].mean() - base_avg_salary:,.0f}$ عن المتوسط العام",
)
...
with tab_overview:
    st.subheader("سباق الرواتب عبر السنين")
    ...
    if race["posting_year"].nunique() > 1:
        ...
        fig.update_layout(showlegend=False,
                           title="اضغط ▶ تحت الرسم عشان تشوف تطوّر الرواتب سنة بسنة")
